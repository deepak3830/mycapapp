...
service CatalogService @(
...
) { // to enable authentication and authorization, so that the user can only
    ...
    entity EmployeeSet @(
        ...
        }, // this will allow the user to view the data if he has the Viewer role, otherwise he will get an error message
        {
            grant: [
                'UPDATE',
                'DELETE',
            ...
            to   : 'Editor'
        } // this will allow the user to update and delete the data if he has the Editor role, otherwise he will get an error message
    ...
    entity PurchaseOrderSet @(
        ...
        // @(odata.draft.enabled: true) adds the draft functionality to the entity, so that the user can create a new purchase order and save it as a draft, and then later on can submit it for approval
        {
            *, // to add all the fields of the entity
            ...
            case
                when OVERALL_STATUS = 'A'
                     then '2' // this was throwing error untill its written in this way
                when OVERALL_STATUS = 'R'
                     then '3'
                when OVERALL_STATUS = 'P'
                     then '1'
